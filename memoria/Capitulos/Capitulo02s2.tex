...
Sea $\Fq$ un cuerpo finito de $q$ elementos, donde $q$ es la potencia $n$-ésima de un primo $p$ y sea $E$ una curva elíptica definida sobre $\Fq$. Como el número de pares $(x, y)$ con $x, y \in \Fq$ es finito, el grupo de puntos $E(\Fq)$ es finito. En este apartado estudiaremos propiedades de este grupo, como el orden, que serán importantes en muchos contextos.
...
$$
con $n_i \mid n_{i+1}$. Como, para cada $i$, el grupo $Z_{n_i}$ tiene $n_1$ elementos de orden $n_1$, tenemos que $E(\Fq)$ tiene $n_1^r$ elementos de orden un divisor de $n_1$. Por el teorema~\ref{th:estructura subgrupos torsión}, hay hasta $n_1^2$ (incluso si permitimos coordenadas en la clausura algebraica de $\Fq$). Por tanto, $r \le 2$.
\end{proof}
...
En este apartado veremos un ejemplo importante de endomorfismo, que es la generalización del endomorfismo de Frobenius sobre cuerpos de caracterítica $p$, y que jugará un papel crucial en la teoría de curvas elípticas sobre cuerpos finitos.
...
\end{proposicion}
\begin{proof}
Veamos primero que $\phiq(x, y) \in \EFqca$. Usaremos las siguientes propiedades de los cuerpos finitos de característica $p$:
\begin{align*}
...
\begin{align*}
	...
	x &\mapsto x^q
\end{align*}
sobre las componentes de un punto. Veamos ahora unas cuantas propiedades de este endomorfismo.
