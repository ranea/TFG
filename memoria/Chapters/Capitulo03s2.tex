...
Se ha utilizado \code{python 3} como lenguaje de programación para \code{ccepy}. La lógica del programa no usa ninguna biblioteca externa, solo se utiliza la biblioteca estándar de \code{python}. Por eso, con tener \code{python} instalado es suficiente para usar \code{ccepy}. En la documentación adjunta se detalla como usar este programa.

Para realizar la documentación se ha utilizado el software \code{sphinx} que permite generar páginas web a partir del código fuente. El proceso de documentación se ha detallado en~\ref{Generación de la documentación}.

Para realizar pruebas basados en propiedades (véase ~\ref{sub:Pruebas}) se ha utilizado la biblioteca \code{hypothesis} que permite generar múltiples casos de prueba a partir de una especificación de propiedades.
...
\end{verbatim}
además de un módulo adicional \code{Listado de curvas elípticas}.
