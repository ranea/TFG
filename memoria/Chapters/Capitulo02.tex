...
\label{sec:Introducción a las curvas elípticas}
...
% TODO: añadir ref
El principal objetivo de este apartado es probar la proposición ~\ref{} que será utilizada en la demostración del teorema de Hasse~\ref{}. Para ello, es necesario utilizar algunos resultados técnicos, los cuales solo los enunciaremos (puede encontrar su demostración en ~\cite{Washington:2008} ).
...
\begin{definicion}
	Sea $E$ una curva elíptica definida sobre un cuerpo $K$ y sea $\Kca$ su clasura algebraica. Un \emph{endomorfismo} de $E$ es un homomorfismo $\alpha: \EKca \to \EKca$ dado por funciones racionales. Esto es, $\alpha$ preserva la suma y el elemento neutro de $\EKca$.

	El endomorfismo trivial que lleva cada punto a $\infty$ lo denotaremos por 0. Supondremos que $\alpha$ es no trivial a partir de ahora.
\end{definicion}
...
\begin{lema}\label{lm:endomorfismo con funciones racionales}
	...
	donde
	\begin{itemize}
		\item $r_1(x) = p(x) / q(x)$, donde $p(x), q(x)$ no tienen factores comunes.
		\item Si $q(x) = 0$ para algún punto $(x, y)$, asumimos que $\alpha(x, y) = \infty$.
