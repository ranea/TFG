...
% TODO: conceptos
% conceptos han aparecido: extensión de cuerpo, característica, grupo, homomorfismo, clasura algebraica
...
En este apartado se tratarán las ecuaciones de Weierstrass, las operaciones de adicción y duplicación, los puntos proyectivos , los endomorfismos de curvas elípticas y los conceptos de singularidad y supersingularidad.
...
\begin{nota}[comentarios de la definición~\ref{def:curva elíptica}]\leavevmode
	\begin{itemize}
		...
		% TODO: rellenar ref
		\item La condición $\Delta \neq 0$ asegura que la curva elíptica es no singular (véase apartado~\ref{})
		% <<suave>>, esto es, no hay puntos en los que la curva tenga dos o mas rectas tangentes.
...
\begin{nota}[comentarios del lema~\ref{lm: simplificación ecuación Weierstrass}] \leavevmode
	\begin{itemize}
		% TODO: rellenar ref
		\item Si el cuerpo base tiene característica 2, la ecuación anterior~\eqref{eq:ecuación Weierstrass} no es válida ya que tiene puntos singulares (véase~\ref{}).
...
En la mayor parte del trabajo, desarollaremos la teoría de curvas elípticas utilizando la ecuación de Weierstrass simplificada~\eqref{eq:ecuación Weierstrass}. Sin embargo, como los cuerpos finitos de característica dos son de especial interés en computación, ocasionalmente señalaremos que modificaciones son necesarias de la para cuerpos de característica dos. Veamos la primera modificación.
...
% TODO: añadir ref
El principal objetivo de este apartado es probar la proposición ~\ref{} que será utilizada en la demostración del teorema de Hasse~\ref{}. Para ello, es necesario utilizar algunos resultados técnicos, los cuales solo los enunciaremos (puede encontrar su demostración en ~\cite[sec. 2.9]{Washington:2008} ).
...
\begin{proof}
	...
	Sea $S$ el conjunto de $x \in \Kca$ tal que $(p q' - p' q)(x) q(x) = 0$. Sea $(a, b) \in \EKca$ tal que
	\begin{enumerate}
		\item $a \neq 0,\ b \neq 0,\ (a, b) \neq \infty$,
		\item $\deg (p(x) - a q(x) ) = \max \{ \deg(p), \deg(q) \} = \dega$,
		\item $a \not\in r_1(S)$ y
		\item $(a, b) \in \alpha(\EKca)$.
	\end{enumerate}
	Como $p' q - p q'$ no es el polinomio nulo, $S$ es un conjunto finito, por lo que su imagen bajo $\alpha$ es finita. Por otro lado $\alpha(\EKca)$ es un conjunto infinito. Así, tal $(a, b)$ existe.

	Veamos que existen exactamente $\dega$ puntos $(x_1, y_1) \in \EKca$ tal que $\alpha(x_1, y_1) = (a, b)$. Para tales puntos, se tiene
	$$
	\frac{p(x_1)}{q(x_1)} = a, \quad y_1 r_2(x_1) = b
	$$
	Como $(a, b) \neq \infty$, $q(x_1) \neq 0$ luego $r_2(x_1)$ está definido. Como $b \neq 0$ y $y_1 r_2(x_1) = b$, se tendrá $y_1 = b/r_2(x_1)$, esto es, $x_1$ determina $y_1$, por lo que solo tenemos que contar valores de $x_1$.

	Por la propiedad (2), $p(x) - a q(x)$ tiene $\dega$ raíces, contando multiplicidades. Tenemos que ver que $p - a q$ no tiene raíces múltiples. Supongamos que $x_0$ es una raíz múltiple. Entonces
	$$
		p(x_0) - a q(x_0) = 0, \quad p'(x_0) - a q'(x_0) = 0
	$$
	Multiplicando las ecuaciones $p = a q$ y $a q' = p'$ resulta
	$$
		a p(x_0) q'(x_0) = a p'(x_0) q(x_0)
	$$
	Como $a \neq 0$, esto implica que $x_0$ es una raíz de $p q' - p' q$, por lo que $x_0 \in S$. Así $a = r_1(x_0) \in r_1(S)$, contrario a la propiedad (3). Concluimos que $p - a q$ no tiene raíces múltiples y por ello tiene $\dega$ raíces distintas.

	Como hay exactamente $\dega$ puntos $(x_1, y_1)$ con $\alpha(x_1, y_1) = (a, b)$, el núcleo de $\alpha$ tiene $\dega$ elementos.

	Nótese que como $\alpha$ es un homomorfismo, para cada $(a, b) \in \alpha(\EKca)$ hay exactamente $\dega$ puntos $(x_1, y_1)$ con $\alpha(x_1, y_1) = (a, b)$. Las hipótesis sobre $(a, b)$ se hicieron para obtener el resultado para al menos un punto, lo cual es suficiente.

	Si $\alpha$ no es separable, entonces los pasos de la demostración siguen siendo válidos, excepto que $p' - a q'$ es siempre el polinomio cero en este caso, por lo que $p(x) - a q(x) = 0$ tiene siempre raíces múltiplces y por ello tiene menos de $\dega$ soluciones.
\end{proof}
...
\subsection{Curvas singulares y supersingulares}
\label{sub:Curvas singulares y supersingulares}
