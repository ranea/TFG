...
En este capítulo haremos el desarrollo informático sobre criptografía con curvas elípticas. En el apartado~\ref{sec:Criptografía con curvas elípticas} hablaremos sobre criptografía asimétrica utilizando curvas elípticas, explicaremos algunos protoclos criptografícos con curvas elípticas en el apartado~\ref{sec:Protocolos criptográficos} y por último en el apartado~\ref{} hablaremos del programa desarrollado para trabajar con curvas elípticas y protocolos criptográficos.
...
Si se siguen estas restricciones para evitar todos los ataques conocidos, se cree que el ECDLP no es factible con la tecnología actual. Nótese que no existe prueba matemática de la intratabilidad del ECDLP, esto es, nadie ha probado que no existe un algoritmo eficiente que lo resuelve. Para apreciar la dificultad de encontrar dicha prueba, la no existencia de un algoritmo en tiempo polinomial para el ECDLP implicaría que $P \neq NP$, una de las preguntas abiertas mas importantes de la teoría de la computación.

Puede más información sobre los ataques al ECDLP en~\cite[cap. 4]{Hankerson:2003} y en~\cite[cap. 4]{Washington:2008}.
...
\begin{protocolo}[ECDSA]
    ...
    \begin{enumerate}
        ...
        \item Alicia representa el documento con un entero $m \in [1, n -1]$ (por ejemplo con una función hash).
...
Si una firma $(r, s)$ de un mensaje $m$ fue generada por el firmador legítimo, entonces $s \equiv k^{-1} (m + d r) \pmod{n}$. Reordando se tiene
$$
...
% TODO: incluir parte de Washington?
